...
class Szene (): #Erstellt Tonnenpaare fuer eine Zielliste, die von der Bahnfuerung abgefagt werden kann
    def __init__(self):
        ...
        self.ziele = [tp1,tp2,tp3,tp4] #Tonnenpaare werden in einem Array zusammengefasst
...
class PIDRegler (): #Klasse fuer die PID Regelung der RPM (Staerke) des Bootsantriebes
  ...
  def berechnen (self,set,act): #Berechnet den Stellwert anhand der Sollgroesse und der Istgroesse
    error = set - act; 
    P = self.kp * error;
...
class Positionierung ():
    ...
    def bootSteuern (self, position):
        ziel=self.ziele[self.aktuellesZiel] #Waehlt aktuelles Ziel aus Array
        zielX=ziel.mittelpunktX
        zielY=ziel.mittelpunktY
        abstandX=zielX-position.position.x #Abstand zum Ziel
        abstandY=zielY-position.position.y
        ...
        if (zielAbstand <=4): #Wenn der Abstand zum Ziel klein genug ist (grosszuegig gewaehlt, damit genug Zeit fuer Kursaenderung)
            self.aktuellesZiel+= 1 #Naechstes Ziel im Array
            print ("Etappenziel erreicht")
        else:
            zielWinkelDiff=zielWinkel-self.heading #Differenz fuer moegliche Ausgabe
            RPMbb=self.reglerBb.berechnen(zielWinkel*-1,self.heading*-1) #Berechnung des Stellwertes Backbord Motor
            RPMstb=self.reglerStb.berechnen(zielWinkel,self.heading) #Berechnung des Stellwertes Steuerbord Motor
    ...
    def callback(self,data): #Funktion wird jedes Mal ausgefuehrt sobald es eine neue Position im Topic gibt
        if self.counter==100: #Um Abtastrate zu begrenzen, wurde hier ein Counter implementiert, nur jeder 100. Wert wird uebernommen
            ...
            if self.aktuellesZiel<4: #So lange ausgefuehren, wie Ziele in dem Array sind
                self.bootSteuern (position)
            self.counter=0
        else:
            self.counter+=1
